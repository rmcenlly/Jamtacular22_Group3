...
function drawBorders() {
    ...
    leftBorder = createSprite(groundPosition + 30, H/2, 1, H);
    leftBorder.immovable = true;
    borders.add(leftBorder);
    
    rightBorder = createSprite(groundPosition + 1370, H/2, 1, H);
    rightBorder.immovable = true;
    borders.add(rightBorder);
    
    topBorder = createSprite(groundPosition + W/2, H/2 - player.sprite.height/2, W, 1);
    topBorder.immovable = true;
    borders.add(topBorder);
    
    bottomBorder = createSprite(groundPosition + W/2, 720, W, 1);
